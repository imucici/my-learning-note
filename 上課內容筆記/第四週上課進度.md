...
-----
* **InsertionSort(插入排序法)**
    *  將數列分為`排序`與`未排序`兩部分，依序將未排序中的第一筆與已排序數列中的數比大小，並將其插入已排序數列中的適當位置。比較時若遇到的值 `>` 正在處理的值，則將值往右移。(下面[操作](#操作)部分有更詳細的步驟解說)
 <br></br>   
...
* **QuickSort(快速排序法)**
   ...
   * 是一種「把大問題分成小問題處理」方法
   * 從數列中任意挑選一個元素當作`基準(pivot)`，將數列一分為二，大於此數放右邊，小於此數放左邊。(下面[操作](#操作)部分有更詳細的步驟解說)
 <br></br>
